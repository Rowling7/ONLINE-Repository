...
SELECT [K01011],
	...
	FROM (
		SELECT
			rtrim(ltrim(replace(replace(replace(replace(K6002,char(13),''),char(10),''),char(9),''),char(32),''))) k6002,		--桥梁名称
			RTRIM(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(K60.K0101,'340000',''),'000000',''),'D001',''),'D002',''),'D003',''))+
			RTRIM(k0108)+
			RTRIM(k0109) K01011
			FROM K60
				LEFT JOIN
					(
						SELECT A.A0102,A.K0101,A.K6001,MAX(B.K0304) K0304,b.k0108,b.k0109
						...
						GROUP BY A.A0102,A.K0101,A.K6001,b.k0108,b.k0109
	...
	WHERE t2.[K01011] = t1.[K01011]
	FOR XML PATH('')), 1, 2, '') AS Combined_k6002
FROM (
	SELECT
		rtrim(ltrim(replace(replace(replace(replace(K6002,char(13),''),char(10),''),char(9),''),char(32),''))) k6002,		--桥梁名称
		RTRIM(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(K60.K0101,'340000',''),'000000',''),'D001',''),'D002',''),'D003',''))+
...
--round(str1,3,1) 对str1 ，保留三位小数，1是指不进位，直接截取
SELECT ROUND(AGE,0) FROM DBO.INFO
select round((K6303+K6305/2000) ,3,1)
