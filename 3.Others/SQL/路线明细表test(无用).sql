...
case when charindex('D',k0101)=11 then  left(k0101,4)
    when rtrim(k0101)like '%D%' then left(rtrim(k0101),len(rtrim(k0101))-4)
    else replace(replace(rtrim(k0101),'000000',''),'420000','')end
from k01
--where left(k0101,1)in ('G','S','H','T')--and k0101 like '%D%'
order by k0101


(charindex('D',k0101) <> 0 or len(k0101)<=9 or left(k0101,1)='X') and left(k0101,1) in ('G','S','X','H','T','J')
(charindex('D',k0101) = 0 and len(k0101)>9) and left(k0101,1) in ('G','S','H','T')


select  k0101
from k01
where  k0101 like '%D___'
--where left(k0101,1)in ('S')and k0101 like 'S%D___'
order by k0101

SELECT LEFT('S22D001',LEN('S22D001')-4)
